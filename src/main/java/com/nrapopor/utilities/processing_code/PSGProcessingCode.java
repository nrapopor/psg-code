...
public class PSGProcessingCode extends PApplet implements ISettingsAware, IRuntimeSettingsAware {
    ...
    private void autonomousMode() {
        ...
        for (int i = 0; i < camWidth * camHeight; i++) {
            ...
            final boolean motion =
                abs(red(currFrame[i]) - red(Background[i])) + abs(green(currFrame[i]) - green(Background[i]))
                    + abs(blue(currFrame[i]) - blue(Background[i])) > 200 - tolerance && trackingMotion;
    ...
    public void retryArduinoConnect() {
        ...
        if (!getSettings().isRunWithoutArduino() && !currentPort.equals("0")) {
            ...
            millisStart = millis();
            while (millis() - millisStart < 5000) { //TODO Why not just sleep here? why are we waiting 5 seconds?
                ;
            }
        }
    ...
    public void setup() {
        ...
        camInput.start(getSettings().getCamWidth(), getSettings().getCamHeight());
