...
//    }
    ...
    for (i = (len-1); i > -1; i--) { //DOING it in reverse (normally the prort is either ttyAMA0 or ttyUSB0 -- if usb I want to be picked up first
      String currPort = Serial.list()[i];
      if (!currPort.startsWith("/dev/ttyA") && !currPort.startsWith("/dev/ttyU")) {
        continue;
      }
      println("Testing port " + currPort);
